...
% ---------------------------------------------
% 'rfori4lesions'           | 1,2,3  | 192   
% 'rforiDRFT4lesions'       | 1,2,3  | 192
% 'cone4lesions'            | 1      | 40
% ---------------------------------------------
...
cnt7.Duration = 200; %changed to 300ms (from 50ms) to see if it improves Jo's performance
scene7 = create_scene(cnt7);

%% Scene 8. time-out if broken fixation (negative punishment)
...
%% Scene 9. Clear fixation cross with goodmonkey
...
if 0==error_type
    run_scene(scene7);    % Run the delay scene (eventmarker 27 - TaskObject - 3 ON) 
    if ~fix5.Success         % The failure of WithThenHold indicates that the subject didn't maintain fixation on the sample image.
        error_type = 3;      % So it is a "break fixation (3)" error.
        run_scene(scene8,[97,36]); % blank screen | 97 = fixation broken, 36 = fix cross OFF
    else
